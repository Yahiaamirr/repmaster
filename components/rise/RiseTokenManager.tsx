    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {event.is_team ? (
          <button
            ...
            disabled={busy}
            className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-[#e8440a] hover:bg-[#c73a08] disabled:opacity-50 text-white rounded-md font-semibold transition-colors"
          >
            <Plus size={14} />
            Generate per-team links
          </button>
        ) : (
          <button
      ...
      {tokens.length === 0 ? (
        <p className="text-zinc-500 text-sm">
          {event.is_team
            ? 'Generate one judge link per team — each opens that team’s rep counter.'
            : 'Add a judge station — one device scores every athlete (tap an athlete, then time/measure).'}
        </p>
      ) : (
        <div className="space-y-3">
          {tokens.map(t => (
            <TokenRow key={t.id} token={t} link={linkFor(t.token)} subtitle={teamName(t.scope?.team_id)} />
          ))}
        </div>
      )}
    </div>
...
function TokenRow({ token, link, subtitle }: { token: RiseJudgeToken; link: string; subtitle?: string }) {
