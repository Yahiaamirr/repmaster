      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-xs text-zinc-500 font-mono flex items-center gap-1.5 truncate">
            <Radio size={9} className="text-[#e8440a] animate-pulse shrink-0" />
            {eventName}
          </p>
          <p className="text-sm font-bold text-white truncate">{label}</p>
        </div>
      </div>
      {right && <span className="text-[10px] font-bold tracking-widest text-[#e8440a] uppercase shrink-0">{right}</span>}
    </div>
  )
}
